...
function renderCart() {
  ...
  $("#cart-items").innerHTML = cart.length ? `
    <h2 class="font-black text-2xl mb-3">Bag</h2>
    ${cart.map((item) => `
      <article class="cart-item">
        ...
        <div>
          ...
          <button class="remove-btn mt-3" data-remove="${item.key}">Remove</button>
        </div>
        <div class="price-row"><strong>${formatMoney(item.price * item.quantity)}</strong></div>
      </article>
    `).join("")}
  ` : `<h2 class="font-black text-2xl">Your cart is empty</h2><p class="muted mt-3">Add a ThreadMaxx piece before checkout.</p><a class="btn-dark mt-5" href="products/">Shop products</a>`;
  ...
  $("#cart-summary").innerHTML = `
    ...
    <a class="btn-primary w-full mt-5" href="checkout.html">Checkout</a>
